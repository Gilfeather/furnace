    ...
    if models_path.exists() {
        ...
        for entry in std::fs::read_dir(models_path).unwrap() {
            ...
            if path.extension().and_then(|s| s.to_str()) == Some("onnx") {
                ...
                println!("cargo:warning=Attempting to generate Rust code for ONNX model: {}", model_name);
                
                // Skip problematic models for now
                if model_name.contains("gptneox") {
                    println!("cargo:warning=Skipping {} due to known compatibility issues", model_name);
                    continue;
                }
                ...
                match std::panic::catch_unwind(|| {
                    ...
                    Ok(_) => {
                        println!("cargo:warning=Successfully generated code for {}", model_name);
                    }
                    Err(_) => {
                        println!("cargo:warning=Failed to generate code for {} - skipping", model_name);
                    }
                }
            }
        }
